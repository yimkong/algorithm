 ...
 */
public class CountStudentsForNoLunch {
    public int countStudents(int[] students, int[] sandwiches) {
        LinkedList<Integer> studentsList = new LinkedList<>();
        Arrays.stream(students).forEach(studentsList::add);
        LinkedList<Integer> sandwichesList = new LinkedList<>();
        Arrays.stream(sandwiches).forEach(sandwichesList::add);
        int count = 0;
        while (count != studentsList.size()) {
            Integer peek = sandwichesList.peek();
            Integer peek1 = studentsList.peek();
            Integer studentLike = studentsList.poll();
            if (peek1.equals(peek)) {
                count = 0;
                sandwichesList.poll();
            } else {
                studentsList.addLast(studentLike);
                count++;
            }
        }
        return count;
